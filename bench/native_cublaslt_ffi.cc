// Development-only native BF16 linear dispatch. Not part of the runtime wheel.
#include <cublasLt.h>
...
ffi::Error NativeLinear(cudaStream_t stream,
                        ffi::BufferR2<ffi::BF16> x,
                        ffi::BufferR2<ffi::BF16> weight,
                        ffi::ResultBufferR2<ffi::BF16> output,
  ...
  CHECK_BLAS(cublasLtMatrixLayoutCreate(&r.a, CUDA_R_16BF,
                                       n == 1 ? 1 : k, n == 1 ? k : n,
                                       n == 1 ? 1 : k));
  CHECK_BLAS(cublasLtMatrixLayoutCreate(&r.b, CUDA_R_16BF, k, m, k));
  CHECK_BLAS(cublasLtMatrixLayoutCreate(&r.d, CUDA_R_16BF, n, m, n));
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(FoldjaxBenchNativeLinear, NativeLinear,
