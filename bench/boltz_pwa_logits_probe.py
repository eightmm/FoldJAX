...
def strided_four_forward(x, kernel):
    """Measured sum-order hypothesis, not a proven cuBLAS kernel identity."""
    import jax
    import jax.numpy as jnp

    if x.shape[-1] != 128 or kernel.shape not in ((128, 1), (128, 8)):
        raise ValueError("requires original 128-wide eight-head projection")
    x = x.astype(jnp.bfloat16).astype(jnp.float32)
    kernel = kernel.astype(jnp.bfloat16).astype(jnp.float32)
    outputs = []
    for h in range(kernel.shape[1]):
        products = jax.lax.optimization_barrier(x * kernel[:, h])
        ...
        outputs.append(total[..., :1].astype(jnp.bfloat16))
...
def candidate(args, reference, x, weight):
    ...
    options = candidate_compile_options(
        "baseline" if args.profile == "strided-four" else args.profile
    )
    forward = (
        strided_four_forward if args.profile == "strided-four" else candidate_forward
    )
    with jax.default_matmul_precision("highest"):
        executable = (
            jax.jit(forward, compiler_options=options)
            .lower(xx, ww)
            .compile()
        )
        outputs = [
...
def main():
    ...
    parser.add_argument(
        "--profile", choices=("baseline", "split-k-1", "no-triton", "strided-four"),
        default="baseline",
    )
