...
def main() -> int:
    ...
    if schedule:
        print(
            ...
            "requested seed 101, one alignment shared by both sides "
            "(historical OpenFold3 upstream seed caveat in notes)\n"
        )
    ...
    for length, case in cases:
        for model in models:
            ...
            usable = (
                fj is not None and up is not None and not (broken(fj) or broken(up))
            )
...
New upstream rows also record the checkout commit, exact reviewed tracked-diff
digest (if any), path-free execution/device identity, and runtime versions; an
unpinned tracked edit is refused before prediction starts.
...
all of it compiling. The measured child follows a successful prefill, and an
eligible readable cache entry can serve the compiled executable. Cache
rejection or deserialization failure can still recompile and is included in
the measured time.
...
Historical OpenFold3 upstream rows recorded before the 2026-08-31 harness fix
have an additional seed caveat: passing 0.3.1's `--num_model_seeds 1`
overrode the YAML's 101 with a value generated from 42. Their time and memory
remain shape-valid, but their confidence is not seed-matched. Current runs use
the YAML seed directly.

A row is `failed` when that side produced no structure, which is not the same
as a non-zero exit. In the historical underprovisioned 970-token upstream
OpenDDE row, it asked for 40.8 GiB of triangle-attention softmax on top of
58.5 GiB already held against PyTorch's 94.97 GiB usable allocation ceiling,
then caught the error, wrote it to an `ERR/` file, and exited 0. Taken at its
word that was a 12-second run that beat everything else in the table.

Every upstream uses the fastest path its reference and this hardware can reach.
Two environments did not at first, and both differences were large enough to
change the conclusions rather than shade them -- see
`bench/upstream-environments.md`:
