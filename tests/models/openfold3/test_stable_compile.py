...
def test_sample_count_partitions_the_stable_jit(monkeypatch) -> None:
    ...
    released_width = inference.released_config(
        n_token=128,
        n_atom=1,
        num_samples=5,
    )
    high_sample = released_width._replace(num_samples=6)

    try:
        released_value = inference.compile_predict(released_width, _table())(*args)
        repeated = inference.compile_predict(released_width, _table())(*args)
...
def test_ambient_triangle_kernel_is_resolved_into_the_graph_identity(
    ...
    def tiny_predict(
        ...
        *,
        ...
        return batch["x"] + {
            "cueq": 1, "xla": 2, "native-private": 3, "cueq-full": 4,
        }[kernel]
    ...
    try:
        ...
        native = run(*args)
        monkeypatch.setenv(TRIANGLE_BACKEND_ENV, "cueq-full")
        full = run(*args)
        jax.block_until_ready((cueq, xla, native, full))

        assert traces == ["cueq", "xla", "native-private", "cueq-full"]
        np.testing.assert_array_equal(cueq, [1])
        np.testing.assert_array_equal(xla, [2])
        np.testing.assert_array_equal(native, [3])
        np.testing.assert_array_equal(full, [4])
        assert inference._compiled_predict._cache_size() == 4
    finally:
        inference._compiled_predict.clear_cache()
