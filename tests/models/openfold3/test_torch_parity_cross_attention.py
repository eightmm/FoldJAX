...
def _module(conditioned: bool):
    ...
    return CrossAttentionPairBias(
        ...
        c_hidden=C_HIDDEN,
        no_heads=HEADS,
        use_ada_layer_norm=conditioned,
        n_query=N_QUERY,
        n_key=N_KEY,
        inf=1e9,
    )
...
def test_unconditioned_matches_torch(
    openfold3_source: Path, randomized, n_valid: int
) -> None:
    torch = _torch()
    module = randomized(_module(conditioned=False))
    a, z, _s, mask = _inputs(torch, n_valid)
    with torch.no_grad():
        expected = module(a=a, z=z, mask=mask)
    params = map_cross_attention_pair_bias(dict(module.state_dict()))
    assert params.linear_ada_out is None
...
def test_conditioned_matches_torch(openfold3_source: Path, randomized) -> None:
    torch = _torch()
    module = randomized(_module(conditioned=True))
    a, z, s, mask = _inputs(torch)
    with torch.no_grad():
        expected = module(a=a, z=z, s=s, mask=mask)
    params = map_cross_attention_pair_bias(dict(module.state_dict()))
    assert params.linear_ada_out is not None
    actual = cross_attention_pair_bias(
        jnp.asarray(a.numpy()),
        jnp.asarray(z.numpy()),
        params,
...
def test_query_and_key_norms_are_separate(openfold3_source: Path) -> None:
    ...
    _torch()
    state = set(_module(conditioned=False).state_dict())
    assert "layer_norm_a_q.weight" in state
    assert "layer_norm_a_k.weight" in state
...
def test_mapper_reports_a_missing_projection(openfold3_source: Path) -> None:
    _torch()
    state = dict(_module(conditioned=False).state_dict())
