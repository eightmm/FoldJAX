...
def test_silent_when_the_job_fits(pool, n_structural, trunk_dtype):
    assert predict_runner._preflight_arena(_features(n_structural), trunk_dtype) is None


def test_fires_on_the_float32_wall(pool):
    """1,531 residues in float32 is the measured OOM: ~93.6 GiB against a ~77 GiB
    budget."""
    message = predict_runner._preflight_arena(_features(N_ST_1531), None)
...
def test_does_not_offer_a_lever_already_spent(pool):
    ...
    given."""
    message = predict_runner._preflight_arena(_features(6000), BF16)
    assert message is not None
...
def test_silent_without_a_pool(monkeypatch):
    ...
    assert predict_runner._preflight_arena(_features(N_ST_1531), None) is None


def test_silent_without_a_structural_token_count(pool):
    assert predict_runner._preflight_arena({}, None) is None
...
def test_estimate_matches_the_measured_arenas(pool):
    ...
    for n_st, dtype, measured in (
        ...
        estimate = predict_runner._ARENA_MIB_PER_PAIR[name] * n_st * n_st
