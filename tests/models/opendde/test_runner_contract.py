"""The seam between the OpenDDE parser and the runner it hands a config to.
...
def test_a_parsed_namespace_builds_the_config_with_no_translation(
    ...
    assert config.diffusion_dtype == "fp32"
...
def test_the_runner_is_tracked_as_a_manifest_source() -> None:
    """The body that changes predictions has to stay bound to resume.

    `cli/predict.py` is tracked because its content changes what an otherwise
    identical request predicts. The body moved to the runner, so the runner is
