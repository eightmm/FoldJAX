"""Print a normalised fingerprint of the serial OpenDDE denoiser lowering.
...
run executes?  The parameters come from
``tests.models.opendde.atom_cp_fixtures``, which is always taken from the tree
under test's *tests* directory, so only ``foldjax`` differs between the two
runs.
...
OpenDDE's own wrapper is the entry point rather than Protenix', because
OpenDDE's conditioning runs first and hands the shared network a
``conditioned_single_s``; the hash of Protenix' denoiser says nothing about
whether that composition still lowers the way it did.

Output: ``HASH <hex>``, ``COLLECTIVES <n>``, ``SHARDING_CALLS <n>``,
``SHARDING_ANNOTATIONS <n>``.
"""
