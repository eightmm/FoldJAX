...
def test_strided_four_matches_explicit_cpu_sum():
    import jax
    import jax.numpy as jnp
    ...
    x = probe.bf16_round(rng.normal(size=(1, 3, 2, 128)).astype(np.float32))
    w = probe.bf16_round(rng.normal(size=(128, 8)).astype(np.float32))
    expected = np.stack([
        probe.bf16_round(ordered_sum(x * w[:, h], 4)) for h in range(8)
    ], axis=-1)
    result = jax.jit(probe.strided_four_forward)(jnp.asarray(x), jnp.asarray(w))
    assert result.dtype == jnp.bfloat16
    np.testing.assert_array_equal(np.asarray(result, np.float32), expected)
