...
| Boltz-2 | 5SAK + ion | 0 | 0 | 5SAK (kernel-toggle scale) | ion 0.17 Å (see below) | `boltz2-master-kernel-toggle-2026-09-09.md` |
...
3. **The port's own floor is XLA kernel selection.** Freezing autotune makes
   every port bitwise repeatable across processes (OpenFold3, ESMFold2
   confirmed; Protenix and Boltz-2 pairs in flight) and removed the one
   1UBQ "residual" on ESMFold2 (0.122 → 0.02 Å).
...
- **ESMFold2 7ST3 chain B**: samples 1/3/5 sit 2-4× above native's own
  ...
  pairing.
- **Boltz-2 ion case (1AAY)**: protein 0.175 Å against a bitwise-repeatable
  native and a 0.018 Å port floor; the kernels-off scale for this case is
  running (job 634). 5SAK remains a chaotic target closed at the
  kernel-toggle scale.
