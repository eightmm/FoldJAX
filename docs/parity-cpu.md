...
`jax_default_matmul_precision="high"` -- OpenDDE's is, and Boltz-2's is since
2026-09-11 -- the CPU replay cannot run it at all: `dot_general` on CPU rejects
`TF32_TF32_F32`, because there is no TF32 there. The CPU arm runs `highest`, so
the GPU arm its residual should be read against is the panel's `highest` arm
rather than the shipped-default one (on OpenDDE `protein_1ubq` sample 0 those
are 0.00178 A and 0.00223 A, and the CPU run lands on the first). Boltz-2's
replay was already written that way: `tests/parity/test_boltz2.py` pins
`highest` in `SHARED_OPTIONS` *and* opens the matching JAX scope, so its
tolerances still describe the arm they were calibrated on. Set the policy with the
