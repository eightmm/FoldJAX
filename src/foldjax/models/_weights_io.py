"""Native JAX weight serialization shared by the Protenix-family ports.

Protenix and OpenDDE write and read the *same* native weight file: one pickled
NumPy tree whose interior nodes are the ports' parameter NamedTuples. The
reader has to admit both ports' parameter modules -- a single OpenDDE
checkpoint carries Protenix classes -- so the implementation belongs to
neither. It used to live in ``protenix.bridge.weights_io``, and
``opendde.bridge.weights_io`` reached into it for the private field-dtype
loader and its batch bound. Both port modules are now facades over this one,
and the admitted module prefixes are stated once, here.

Parameter classes are resolved by name through :mod:`pickle`, so nothing here
imports a port -- or torch -- to read a checkpoint.
"""
...
_PROTENIX_MODELS = "foldjax.models.protenix.models."
# Weight files predating the flat -> grouped module split inside the Protenix
# port. The rewrite is Protenix-only: OpenDDE's parameter modules sit flat
# under ``opendde/models/``, so its classes are read at the names they were
# written under.
_LEGACY_MODULE_ALIASES = {
    f"{_PROTENIX_MODELS}{flat}": f"{_PROTENIX_MODELS}{grouped}"
    for flat, grouped in {
        "atom": "diffusion.atom",
