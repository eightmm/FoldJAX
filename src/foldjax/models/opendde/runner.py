"""The OpenDDE prediction run, as a configuration and one entry point.
...
class PredictionConfig(NamedTuple):
    """One resolved OpenDDE run, one field per option the CLI parses.
    ...
    components_cif: Path | None
    ccd_rdkit_cache: Path | None
...
def run_prediction(
    ...
    *,
    ...
    if config.weights.suffix.lower() in {".pt", ".pth", ".ckpt"}:
        raise SystemExit(
            "prediction requires native JAX weights; run "
            "opendde-jax-export-weights first"
        )
    if not config.input_json.is_file():
        raise SystemExit(f"missing input JSON: {config.input_json}")
    if not config.weights.is_file():
        raise SystemExit(f"missing native weights: {config.weights}")
    if config.num_samples < 1 or config.num_steps < 1 or config.num_recycles < 1:
    ...
    try:
        ...
        if config.confidence_dtype == "bf16":
            import jax.numpy as jnp
        ...
        diffusion_autocast = _resolve_diffusion_autocast(
            config.diffusion_dtype, config.trunk_dtype
        )
        ...
        for job in jobs:
            job_name = str(job.get("name") or config.input_json.stem)
            for seed in _job_seeds(job, config.seed):
                from foldjax.models.opendde.postprocess import (
                ...
                output = _predict(
                    ...
                    trunk_single_attention_backend=(
                        config.trunk_single_attention_backend
                    ),
                    structural_single_attention_backend=(
                        config.structural_single_attention_backend
                    ),
                    trunk_dtype=trunk_dtype,
                    confidence_dtype=confidence_dtype,
                    diffusion_autocast=diffusion_autocast,
                    ...
                    return_confidence_logits=(
                        config.include_raw or "asym_id" not in model_features
                    ),
                    return_confidence_details=config.include_raw,
